...
Detects OS and ENV to choose the right server:
  - Windows / ENV=dev  → Uvicorn with --reload
  - Linux / ENV=prod   → Gunicorn with Uvicorn workers
...
def main() -> None:
    ...
    if env == "prod" and not is_windows:
        # ── Linux production: Gunicorn + Uvicorn workers ──
        ...
        # ── Windows / dev: Uvicorn with reload ──
